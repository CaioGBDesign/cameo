...
export default function AdmEditarResenha() {
  ...
  useEffect(() => {
    ...
    const carregar = async () => {
      try {
        ...
        setClassificacao(data.classificacao != null ? data.classificacao : "");
  ...
  const salvarComSlug = async (campos) => {
    ...
    if (novoSnap.exists())
      throw new Error(`Já existe uma resenha com o slug "${novoSlug}"`);
  ...
  const salvarRascunho = async () => {
    ...
    try {
      ...
      if (novoSlug !== slug) {
        router.replace(`/adm/resenhas/editar/${novoSlug}`);
        return;
      }
      setHasChanges(false);
  ...
  const confirmarAgendamento = async () => {
    ...
    try {
      ...
      toast.error("Erro ao agendar a resenha", {
        duration: 3000,
        buttons: [{ label: "Fechar" }],
        bg: "var(--primitive-erro-01)",
      });
  ...
  const retornarRascunho = async () => {
    try {
      ...
      setStatusAtual("rascunho");
      setMenuAberto(false);
      toast.success("Resenha retornada para rascunho", {
        duration: 3000,
        buttons: [{ label: "Fechar" }],
        bg: "var(--primitive-verde-01)",
      });
  ...
  const arquivar = async () => {
    try {
      ...
      setMenuAberto(false);
      toast.success("Resenha arquivada com sucesso", {
        duration: 3000,
        buttons: [{ label: "Fechar" }],
        bg: "var(--primitive-verde-01)",
      });
      ...
      toast.error("Erro ao arquivar a resenha", {
        duration: 3000,
        buttons: [{ label: "Fechar" }],
        bg: "var(--primitive-erro-01)",
      });
  ...
  const deletar = async () => {
    try {
      ...
      toast.error("Erro ao deletar a resenha", {
        duration: 3000,
        buttons: [{ label: "Fechar" }],
        bg: "var(--primitive-erro-01)",
      });

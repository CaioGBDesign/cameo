...
export async function getServerSideProps({ query }) {
  const { id } = query;
  if (!id) return { props: { seoData: null } };

  try {
    const apiKey = process.env.NEXT_PUBLIC_TMDB_API_KEY;
    if (!apiKey) return { props: { seoData: null } };
    ...
    if (!tmdbRes.ok) return { props: { seoData: null } };
    ...
    return {
      props: {
        seoData: {
          ...
          generos,
          nomesDubladores,
          canonicalUrl: `https://cameo.fun/filme-aleatorio?id=${id}`,
        },
      },
    };
  } catch {
...
export default function FilmeAleatorio({ seoData }) {
  ...
  const pageDescription =
    seoData?.descricao ||
    "Descubra um filme aleatório e veja quem faz parte do elenco de dublagem no Cameo.";

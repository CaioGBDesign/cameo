...
const ModalAvaliar = dynamic(() => import("@/components/modais/avaliar-filmes"));
...
export async function getServerSideProps({ params }) {
  ...
  try {
    ...
    const [tmdbRes, filmesSnap] = await Promise.all([
      ...
      getDocs(fsQuery(collection(db, "filmes"), where("idFilme", "==", parseInt(id)))),
    ]);
...
export default function FilmeDetalhe({ seoData, initialId }) {
  ...
  const abrirModalLista = () => {
    if (!user) {
      if (isMobile) { router.push("/login"); return; }
      openModal("login");
      return;
    }
    ...
    const selecao = {};
    listasCustom.forEach((l) => { selecao[l.id] = l.filmes?.includes(sid) ?? false; });
  ...
  const confirmarLista = async () => {
    ...
    if (selecionarFavorito && !jaFavorito) { salvarFilme(sid); setFavoritosList((p) => [...p, sid]); }
    else if (!selecionarFavorito && jaFavorito) { removerFilme(sid); setFavoritosList((p) => p.filter((f) => f !== sid)); }
    const jaParaVer = assistList.includes(sid);
    if (selecionarParaVer && !jaParaVer) { assistirFilme(sid); setAssistList((p) => [...p, sid]); }
    else if (!selecionarParaVer && jaParaVer) { removerAssistir(sid); setAssistList((p) => p.filter((f) => f !== sid)); }
  ...
  const fetchMovie = useCallback(async (movieId) => {
    ...
    try {
      ...
      setServicosDisponiveis(data["watch/providers"]?.results?.BR?.flatrate || []);
  ...
  const jsonLd = seoData
    ? {
        ...
        ...(seoData.nomesDubladores.length > 0 && {
          actor: seoData.nomesDubladores.map((nome) => ({ "@type": "Person", name: nome })),
        }),
    ...
    <>
      ...
      <Head>
        ...
        {seoData && (
          <meta
            name="keywords"
            content={[seoData.titulo, "dublagem", ...seoData.generos, ...seoData.nomesDubladores.slice(0, 5)].join(", ")}
          />
        )}
        <link rel="canonical" href={seoData?.canonicalUrl ?? "https://cameo.fun/filme-aleatorio"} />
        ...
        <meta property="og:url" content={seoData?.canonicalUrl ?? "https://cameo.fun/filme-aleatorio"} />
        {seoData?.posterUrl && <meta property="og:image" content={seoData.posterUrl} />}
        {seoData?.posterUrl && <meta property="og:image:alt" content={seoData.titulo} />}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={pageTitle} />
        <meta name="twitter:description" content={pageDescription} />
        {seoData?.posterUrl && <meta name="twitter:image" content={seoData.posterUrl} />}
        {jsonLd && (
          <script
      ...
      <main className={styles.container}>
        ...
        {!loading && filme && (
          <>
            ...
            />
            {isMobile ? (
              <SectionCard title="Sinopse" verTodos={{ label: "Ver mais", onClick: () => openModal("sinopse") }}>
            ...
            <div className={styles.acoes}>
              <div className={styles.btnAvaliacao}>
                {!user ? (
                  <Button variant="outline" label="Já assisti" href="/login" border="var(--stroke-base)" arrowColor="var(--stroke-base)" width="100%" />
                ) : user?.visto?.[filme.id] ? (
                  <Button variant="outline" stars={user.visto[filme.id].nota} onClick={() => openModal("rating")} border="var(--stroke-base)" arrowColor="var(--stroke-base)" width="100%" />
                ) : (
                  <Button variant="outline" label="Já assisti" onClick={() => openModal("rating")} border="var(--stroke-base)" arrowColor="var(--stroke-base)" width="100%" />
                )}
              </div>
              <div className={styles.btnListas}>
                <Button variant="submit" label="Adicionar à lista" icon={<AddToListIcon size={20} color="currentColor" />} onClick={abrirModalLista} width="100%" />
              </div>
            </div>
            ...
            <div className={styles.todasAsInformacoes}>
              ...
              {elencoDublagem.length > 0 && (
                <SectionCard title="Elenco de dublagem" scrollRef={elencoDoblagemRef}>
                  <ElencoDublagem ref={elencoDoblagemRef} items={elencoDublagem} />
                </SectionCard>
              )}
                ...
                <SectionCard title="Produção">
                  <div className={styles.producoes}>
                    {filme.production_companies.map((c) => (
                      <div key={c.id} className={styles.producaoItem}>{c.name}</div>
                    ))}
                  </div>
                </SectionCard>
              )}
              <SectionCard title="Recomendações" scrollRef={recomendacoesRef}>
                <div ref={recomendacoesRef} style={{ display: "flex", gap: "var(--space-sm)", overflowX: "auto", overflowY: "hidden", scrollbarWidth: "none" }}>
                  {related.map((movie, idx) => (
                    <CardFilme key={movie.id} movie={movie} variant="titulo" onClick={() => setModalDetalhes({ aberto: true, index: idx })} />
                  ))}
                </div>
              </SectionCard>
            </div>
            ...
            {modalType === "sinopse" && (
              <Modal title="Sinópse completa" onClose={closeModal}>
                <div style={{ background: "var(--bg-overlay)", padding: "var(--space-lg)", borderRadius: "var(--space-lg)", border: "1px solid var(--stroke-base)" }}>
                  <p style={{ fontSize: 16, color: "var(--text-base)", lineHeight: "24px" }}>{filme.overview}</p>
                </div>
              </Modal>
            )}
            {modalType === "rating" && (
              <ModalAvaliar filmeId={filme.id} nota={user?.visto?.[filme.id]} onClose={closeModal} />
            )}
          </>
        )}
      </main>
      ...
      {modalType === "filters" && (
        <ModalFiltros user={user} onClose={closeModal} onSelectMovie={(id) => router.push(`/filme-aleatorio/${id}`)} />
      )}
      ...
      {modalListaAberto && (
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-sm)" }}>
            <CheckboxCard id="modal-favorito" variant="card" label="Adicionar aos favoritos" checked={selecionarFavorito} onChange={(e) => setSelecionarFavorito(e.target.checked)} icon={<BookmarkIcon size={18} color="currentColor" />} />
            <CheckboxCard id="modal-para-ver" variant="card" label="Quero assistir" checked={selecionarParaVer} icon={<ListIcon size={18} color="currentColor" />} onChange={(e) => setSelecionarParaVer(e.target.checked)} />
            {listasCustom.map((lista, idx) => (
              <CheckboxCard
                ...
                onChange={(e) => setSelecaoListas((prev) => ({ ...prev, [lista.id]: e.target.checked }))}
                icon={
                  <div style={{ width: 18, height: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ width: 7, height: 7, borderRadius: 10, background: `var(${LISTA_CORES[idx % LISTA_CORES.length]})` }} />
                  </div>
                }
              />
            ))}

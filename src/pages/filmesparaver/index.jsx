...
export default function filmesParaVer() {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(user?.assistir)) {
      // “para assistir” salvo como array de strings
      ids = user.assistir;
    } else if (user?.assistir && typeof user.assistir === "object") {
      // caso fosse objeto, pegaria as chaves
      ids = Object.keys(user.assistir);
    }
    ...
    Promise.all(
      ids.map((id) =>
        fetch(
          ...
          .then((data) => ({
            ...
            avaliacao: {
              nota: Array.isArray(user.assistir) ? null : user.assistir[id],
            },
          }))
      )
    )
      ...
      .catch((err) => {
        console.error("Erro ao buscar filmes “para assistir”:", err);
        setLoading(false);
      });
  }, [user?.assistir, apiKey]);

...
export default function Tipo2({ id = null, initialData = null }) {
  ...
  // Cast — elencoDisponivel é lista completa do TMDB; elencoSelecionadoIds são os IDs escolhidos
  const [elencoDisponivel, setElencoDisponivel] = useState([]);
  const [elencoSelecionadoIds, setElencoSelecionadoIds] = useState(
    (initialData?.elenco ?? []).map((a) => String(a.id)),
  );
  ...
  // TMDB: busca detalhes + elenco quando idFilme muda
  useEffect(() => {
    if (!idFilme) {
      setElencoDisponivel([]);
      return;
    ...
    const timer = setTimeout(async () => {
      setLoadingCast(true);
      try {
        const [resDetails, resCast] = await Promise.all([
          fetch(
            `https://api.themoviedb.org/3/movie/${idFilme}?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=pt-BR`,
          ),
          fetch(
            `https://api.themoviedb.org/3/movie/${idFilme}/credits?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=pt-BR`,
          ),
        ]);
  ...
  const elencoOptions = elencoDisponivel.map((a) => ({
    value: String(a.id),
    label: a.name,
  }));

  // Objetos completos dos atores selecionados (para salvar e para o preview)
  const elencoSelecionado = elencoSelecionadoIds
    .map((sid) => elencoDisponivel.find((a) => String(a.id) === sid))
    .filter(Boolean);

  const handleChangeElenco = (novoIds) => {
    setElencoSelecionadoIds(novoIds);
    setErros((p) => ({ ...p, elenco: false }));
  };
  ...
  const validar = () => {
    ...
    if (elencoSelecionadoIds.length === 0) e.elenco = true;
  ...
  const sidebar = (
    <div className={styles.sidebarContent}>
      <TextInput
        ...
        placeholder='Ex: "Identifique o filme pelo elenco"'
        value={titulo}
        onChange={(e) => {
      ...
      <div>
        <MultiSelect
          label={`Elenco${elencoSelecionadoIds.length > 0 ? ` (${elencoSelecionadoIds.length} selecionados)` : ""}`}
          options={elencoOptions}
          selected={elencoSelecionadoIds}
          onChange={handleChangeElenco}
          placeholder={
            !idFilme
              ? "Digite o ID do filme primeiro"
              : loadingCast
                ? "Carregando elenco..."
                : elencoOptions.length === 0
                  ? "Nenhum ator com foto encontrado"
                  : "Selecione os atores"
          }
          width="100%"
        />
        {erros.elenco && (
          <p className={styles.erroMsg}>Selecione ao menos um ator/atriz</p>
        )}
      </div>
  ...
  // ── Preview ────────────────────────────────────────────────────────────────
  const castPreview =
    elencoSelecionado.length > 0 ? elencoSelecionado : [null, null, null, null];
    ...
    <div className={styles.previewWrapper}>
      <div className={styles.previewCard}>
        ...
        <div className={styles.previewCastGrid}>
          {castPreview.map((ator, i) => (
            <div
              key={ator?.id ?? `placeholder-${i}`}

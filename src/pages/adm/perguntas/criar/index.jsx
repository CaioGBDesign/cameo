...
export default function CriarPergunta() {
  ...
  const tipo = router.isReady
    ? router.query.tipo
      ? parseInt(router.query.tipo)
      : null
    : undefined;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        // silencioso — ID inválido ou sem conexão
      } finally {
  ...
  const handleSalvar = async (status = "rascunho") => {
    ...
    try {
      ...
      await setDoc(doc(db, "perguntas", id), dados);
  ...
  return (
    <>
      <Head>
        <title>Criar pergunta — Cameo ADM</title>
      </Head>

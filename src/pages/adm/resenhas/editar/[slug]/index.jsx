...
const IcoPublicar = () => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <defs><clipPath id="pub-ed"><rect width="16" height="16" fill="white"/></clipPath></defs>
  </svg>
);
...
const generoOptions = generosList.map((g) => ({ value: g.name, label: g.name }));
const empresaOptions = empresasList.map((e) => ({ value: e.name, label: e.name }));

export default function AdmEditarResenha() {
  useAuth();
  ...
  useEffect(() => {
    ...
    const carregar = async () => {
      try {
        const docSnap = await getDoc(doc(db, "criticas", slug));
        if (!docSnap.exists()) {
        ...
        if (data.tagIds?.length) {
          ...
          const todasTags = tagsSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
  ...
  const uploadImagem = async () => {
    ...
    const storageRef = ref(storage, `criticas/${Date.now()}_${imagemElemento.file.name}`);
  ...
  const salvarComSlug = async (campos) => {
    ...
    if (novoSlug === slug) {
      await updateDoc(doc(db, "criticas", slug), campos);
      return slug;
    }
    const novoDocRef = doc(db, "criticas", novoSlug);
    const novoSnap = await getDoc(novoDocRef);
    if (novoSnap.exists()) throw new Error(`Já existe uma resenha com o slug "${novoSlug}"`);
    const atualSnap = await getDoc(doc(db, "criticas", slug));
    const atualData = atualSnap.data();
    await setDoc(novoDocRef, { ...atualData, ...campos });
    await deleteDoc(doc(db, "criticas", slug));
    return novoSlug;
  };

  const salvarRascunho = async () => {
    if (!titulo.trim()) {
      toast.warn("Adicione um título antes de salvar", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-amarelo-02)" });
      return;
    }
    setLoadingRascunho(true);
    try {
      const campos = await getCamposBase();
      const novoSlug = await salvarComSlug({ ...campos, status: "rascunho", dataRascunho: serverTimestamp() });
      if (novoSlug !== slug) { router.replace(`/adm/resenhas/editar/${novoSlug}`); return; }
      setHasChanges(false);
      toast.success("Rascunho salvo com sucesso", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-verde-01)" });
    } catch (err) {
      console.error("Erro ao salvar rascunho:", err);
      toast.error("Erro ao salvar rascunho", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-erro-01)" });
    } finally {
  ...
  const publicar = async (e) => {
    ...
    if (!titulo.trim() || !numero) {
      toast.warn("Complete as informações para publicar", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-amarelo-02)" });
      return;
    }
    setLoading(true);
    try {
      const campos = await getCamposBase();
      await salvarComSlug({ ...campos, status: "publicado", dataPublicacao: serverTimestamp() });
      router.push("/adm/resenhas");
    } catch (err) {
      console.error("Erro ao publicar:", err);
      toast.error("Erro ao publicar a resenha", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-erro-01)" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const salvarAlteracoes = async () => {
    if (!titulo.trim()) { toast.warn("Adicione um título antes de salvar"); return; }
    setLoadingAlteracoes(true);
    try {
      const campos = await getCamposBase();
      const novoSlug = await salvarComSlug({ ...campos, dataEditado: serverTimestamp() });
      if (novoSlug !== slug) { router.replace(`/adm/resenhas/editar/${novoSlug}`); return; }
      setHasChanges(false);
      toast.success("Alterações salvas com sucesso", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-verde-01)" });
    } catch (err) {
      console.error("Erro ao salvar alterações:", err);
      toast.error("Erro ao salvar as alterações", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-erro-01)" });
    } finally {
  ...
  const confirmarAgendamento = async () => {
    if (!dataAgendado || !horaAgendado) {
      toast.warn("Informe a data e hora para agendar", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-amarelo-02)" });
      return;
    }
    try {
      const dataAgendamento = new Date(`${dataAgendado}T${horaAgendado}`);
      await updateDoc(doc(db, "criticas", slug), { status: "agendado", dataAgendamento: dataAgendamento.toISOString() });
      setStatusAtual("agendado");
      setAgendarAberto(false);
      toast.success("Resenha agendada com sucesso", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-verde-01)" });
    } catch (err) {
      console.error("Erro ao agendar:", err);
      toast.error("Erro ao agendar a resenha", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-erro-01)" });
    }
  };

  const cancelarAgendamento = async () => {
    try {
      await updateDoc(doc(db, "criticas", slug), { status: "rascunho", dataAgendamento: null });
      setStatusAtual("rascunho");
      setMenuAberto(false);
      toast.success("Agendamento cancelado", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-verde-01)" });
    } catch (err) {
      console.error("Erro ao cancelar agendamento:", err);
      toast.error("Erro ao cancelar o agendamento", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-erro-01)" });
    }
  };

  const retornarRascunho = async () => {
    try {
      await updateDoc(doc(db, "criticas", slug), { status: "rascunho" });
      setStatusAtual("rascunho");
      setMenuAberto(false);
      toast.success("Resenha retornada para rascunho", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-verde-01)" });
    } catch (err) {
      console.error("Erro ao retornar para rascunho:", err);
      toast.error("Erro ao retornar para rascunho", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-erro-01)" });
    }
  };

  const arquivar = async () => {
    try {
      await updateDoc(doc(db, "criticas", slug), { status: "arquivado" });
      setStatusAtual("arquivado");
      setMenuAberto(false);
      toast.success("Resenha arquivada com sucesso", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-verde-01)" });
    } catch (err) {
      console.error("Erro ao arquivar:", err);
      toast.error("Erro ao arquivar a resenha", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-erro-01)" });
    }
  };
  ...
  const deletar = async () => {
    try {
      ...
      toast.error("Erro ao deletar a resenha", { duration: 3000, buttons: [{ label: "Fechar" }], bg: "var(--primitive-erro-01)" });
    }
  };
  ...
  const itemMenu = (label, onClick, Icon, danger = false) => (
    <button
      ...
      onClick={() => { onClick(); setMenuAberto(false); }}
    >
  ...
  const visualizarItem = itemMenu(
    "Visualizar",
    () => window.open(`/resenhas/detalhes/${slug}?preview=true`, "_blank"),
    IcoVisualizar,
  );
  ...
  switch (statusAtual) {
    case "rascunho":
      headerActions = (
        <>
          {renderMenu(<>{visualizarItem}{itemMenu("Deletar", deletar, IcoDeletar, true)}</>)}
          <Button variant="ghost" label="Agendar" type="button" onClick={() => setAgendarAberto(true)} border="var(--stroke-base)" />
          ...
          <Button variant="ghost" label={loading ? "Publicando..." : "Publicar"} type="button" onClick={publicar} disabled={loading} border="var(--stroke-base)" />
        </>
      );
      break;
        ...
        <>
          {renderMenu(
            <>
              ...
              {itemMenu("Cancelar agendamento", cancelarAgendamento, IcoCancelarAgendamento)}
              {itemMenu("Arquivar", arquivar, IcoArquivar)}
            </>,
          )}
          <Button variant="ghost" label="Editar agendamento" type="button" onClick={() => setAgendarAberto(true)} border="var(--stroke-base)" />
          <Button variant="ghost" label={loadingAlteracoes ? "Salvando..." : "Salvar alterações"} type="button" disabled={loadingAlteracoes} onClick={salvarAlteracoes} border="var(--stroke-base)" />
        </>
      );
      break;
      ...
      headerActions = (
        <>
          {renderMenu(
            <>
              {visualizarItem}
              {itemMenu("Retornar para rascunho", retornarRascunho, IcoRascunho)}
              {itemMenu("Arquivar", arquivar, IcoArquivar)}
            </>,
          )}
          <Button variant="ghost" label={loadingAlteracoes ? "Salvando..." : "Salvar alterações"} type="button" disabled={loadingAlteracoes} onClick={salvarAlteracoes} border="var(--stroke-base)" />
    ...
    case "arquivado":
      headerActions = (
        <>
          ...
          <Button variant="ghost" label="Retornar para rascunho" type="button" onClick={retornarRascunho} border="var(--stroke-base)" />
        </>
      );
      break;
  ...
  const breadcrumb = [
    { href: "/adm/resenhas", label: "Resenhas" },
  ...
  const sidebar = (
    <>
      ...
      <div className={styles.row}>
        <MultiSelect options={generoOptions} selected={generos} onChange={setGeneros} placeholder="Selecione gêneros…" width="100%" />
        <MultiSelect options={empresaOptions} selected={empresas} onChange={setEmpresas} placeholder="Selecione empresas…" width="100%" />
      </div>
      <div className={styles.row}>
  ...
  return (
    <AdmLayout headerActions={headerActions} breadcrumb={breadcrumb} rightSidebar={sidebar}>
      <Head>
        <title>Cameo ADM — Editar resenha</title>
      </Head>
      <form id="form-resenha-editar" className={styles.form}>
        <AdmEditor
          value={conteudo}
          onChange={(v) => {
            setConteudo(v);
          ...
          <div className={styles.agendarModal}>
            ...
            <div className={styles.agendarCampos}>
              <label className={styles.agendarLabel}>
                Data
                <input type="date" className={styles.agendarInput} value={dataAgendado} onChange={(e) => setDataAgendado(e.target.value)} />
              </label>
              <label className={styles.agendarLabel}>
                Hora
                <input type="time" className={styles.agendarInput} value={horaAgendado} onChange={(e) => setHoraAgendado(e.target.value)} />
              </label>
            </div>
            <div className={styles.agendarAcoes}>
              <Button variant="ghost" label="Cancelar" type="button" onClick={() => setAgendarAberto(false)} border="var(--stroke-base)" />
              <Button variant="ghost" label="Confirmar" type="button" onClick={confirmarAgendamento} border="var(--stroke-base)" />
            </div>
          </div>
        </div>
      )}
